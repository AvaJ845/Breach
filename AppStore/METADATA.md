# App Store — Breach Kit

## Identity — Discovery (keyword-first, per the ASO playbook)
- **App Store Name (≤30):** `Breach Tracker - Breach Kit`  *(26 chars — primary keyword FIRST, brand second)*
  - Pattern match to `Habit Tracker - Habit Kit`. Home Screen name stays `Breach Kit` (`CFBundleDisplayName`).
- **Subtitle (≤30):** `Settlements & class actions`  *(28 chars — all new words, none repeated from the Name)*
  - Alts: `Claims, alerts & identity` *(26)* · `Payouts & breach alerts` *(24)*
- **Bundle ID:** com.avaresearch.breachkit
- **Primary category:** Utilities  ·  **Secondary:** Finance
- **Age rating:** 4+ (no objectionable content)
- **Price:** Free (v1). Optional Pro later for unlimited watches / richer alerts.

## Promotional text (≤170)
Track data-breach settlements and claim deadlines in a calm Wallet — private, on-device, no account. Estimated recovery, reminders, and status in one place.

## Keywords (≤100) — the hidden backend array
`claim,payout,privacy,identity,leak,hack,monitor,scan,exposed,compensation,alert,lawsuit`  *(90 chars)*

Rules applied: comma-separated, **no spaces**, **no word repeated** from Name/Subtitle, **singulars only**, **no competitor names** (no “Payout”). Indexed phrases include `breach claim`, `settlement alert`, `class action payout`, `identity monitor`, `data leak scan`.

## Description
Breach Kit helps you **find, watch, and claim** class-action and data-breach settlements — with the clarity of a Wallet, not the noise of a payday ad.

**Wallet-first**
• Estimated recovery for everything you’re watching or claimed.
• In-progress and finished lists with deadlines and status.
• Gentle local reminders three days before a deadline.

**Private by design**
• No account. No tracking.
• Email scans run on-device against a curated catalog.
• Notes stay on your iPhone.

**Built for trust**
• Clear “no proof needed” vs “proof may help” badges.
• Official claim-site links when available.
• Honest framing: we organize public settlement info — we don’t file claims or guarantee payment.

## What's New (1.0)
First release: Wallet overview, settlement catalog, private on-device email scan, deadline reminders, and happy-moment review prompts.

## URLs
- **Support / Marketing:** https://avaj845.github.io/BreachKit/
- **Privacy Policy:** https://avaj845.github.io/BreachKit/privacy.html

## App Privacy (nutrition label)
- **Data collected:** None. No account, no analytics SDKs.
- Optional local notifications for deadlines you watch.
- Email addresses used for scans are stored only on-device if you save them.

## Review notes (paste into App Review)
Breach Kit is a **personal organizer** for publicly described settlement opportunities. Sample catalog data is illustrative for demo/review. The app does not file claims, provide legal advice, or connect to third-party claims administrators beyond opening user-tapped Safari links.
